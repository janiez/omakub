...
cd /tmp
wget -O nvim.tar.gz "https://github.com/neovim/neovim/releases/download/stable/nvim-linux-arm64.tar.gz"
tar -xf nvim.tar.gz
sudo install nvim-linux-arm64/bin/nvim /usr/local/bin/nvim
sudo cp -R nvim-linux-arm64/lib /usr/local/
sudo cp -R nvim-linux-arm64/share /usr/local/
rm -rf nvim-linux-arm64 nvim.tar.gz
cd -
