...
if [ -f /etc/apt/sources.list.d/spotify.list ]; then
  ...
  echo "deb [arch=amd64 signed-by=/etc/apt/trusted.gpg.d/spotify.gpg] https://repository.spotify.com stable non-free" | sudo tee /etc/apt/sources.list.d/spotify.list
fi